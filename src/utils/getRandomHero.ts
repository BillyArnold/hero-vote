...
export const getRandomHero: (notThisOne?: number) => number = (notThisOne) => {
    const numberOfHeros = heroArrayID.length;

    const HeroNumber = Math.floor(Math.random() * numberOfHeros) + 0;
    const finalHeroID = heroArrayID[HeroNumber];

    if (finalHeroID !== notThisOne) return finalHeroID;
